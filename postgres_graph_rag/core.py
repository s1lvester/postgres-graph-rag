...
class PostgresGraphRAG:
    ...
    def setup(self):
        """Initializes the database schema with the correct embedding dimension."""
    ...
    def ingest(
        ...
        for chunk in chunks:
            ...
            for triplet in triplets:
                ...
                sub_id = self.db.upsert_node(triplet.subject, sub_emb)
                obj_emb = self.extractor.get_embedding(triplet.object)
                obj_id = self.db.upsert_node(triplet.object, obj_emb)
                self.db.upsert_edge(sub_id, obj_id, triplet.predicate)

    def query(self, question: str, hops: int = 2, top_k: int = 5) -> str:
        query_emb = self.extractor.get_embedding(question)
        seed_nodes = self.db.vector_search(query_emb, top_k=top_k)
        seed_ids = [n["id"] for n in seed_nodes]
        graph_data = self.db.traverse_graph(seed_ids, max_hops=hops)
